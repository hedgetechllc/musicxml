use crate::datatypes::{
  ...
  NumberOrNormal, RotationDegrees, SmuflGlyphName, Tenths, TextDirection, Valign,
};
...
pub struct CreditSymbolAttributes {
  ...
  /// Specifies text tracking. Values are either normal, which allows flexibility of `letter_spacing` for purposes of text justification,
...
/// The [CreditSymbol] element is similar to the [Symbol][super::Symbol] element for a [Direction][super::Direction].
///
...
/// to the bottom left-hand corner of the page. The `enclosure` is none if not specified.
///
...
pub struct CreditSymbol {
  ...
  pub attributes: CreditSymbolAttributes,
