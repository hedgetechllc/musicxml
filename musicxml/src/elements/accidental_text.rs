use crate::datatypes::{
  AccidentalValue, Color, EnclosureShape, FontFamily, FontSize, FontStyle, FontWeight, LeftCenterRight, NumberOfLines,
  NumberOrNormal, RotationDegrees, SmuflAccidentalGlyphName, Tenths, TextDirection, Valign, XmlLang, XmlSpace,
};
...
#[cfg(test)]
mod accidental_text_tests {
  use super::*;
  use crate::parser::parse_from_xml_str;

  #[test]
  fn deserialize_valid1() {
    let result = parse_from_xml_str::<AccidentalText>(
      "<accidental-text halign=\"right\" valign=\"baseline\" underline=\"3\" rotation=\"123.3\">double-sharp</accidental-text>");
    assert!(result.is_ok());
    assert_eq!(
      result.unwrap(),
      AccidentalText {
        attributes: AccidentalTextAttributes {
          ...
          underline: Some(NumberOfLines(3)),
          rotation: Some(RotationDegrees(123.3)),
          ..Default::default()
        },
        content: AccidentalValue::DoubleSharp,
      }
    );
  }
}
